...
from utils import fusion_tensor_mask
...
class StableDiffusion3TryOnPipeline(DiffusionPipeline, SD3LoraLoaderMixin, FromSingleFileMixin):
    ...
    Args:
        ...
        aflow ([`AppearanceFlowEncDec`]):
            Appearance flow encoding network composed of four layers of convolution.
    """
    ...
    def __call__(
        ...
        cloth_image_embeds = self._get_clip_image_embeds(cloth_image_vit, num_images_per_prompt, device)
        cloth_image_embeds = cloth_image_embeds.to(device=device)
        cloth_image_embeds = cloth_image_embeds.repeat(num_images_per_prompt, *([1] * (cloth_image_embeds.dim() - 1)))
        
        if self.do_classifier_free_guidance:
            cloth_image_embeds = torch.cat([torch.zeros_like(cloth_image_embeds), cloth_image_embeds], dim=0)
        ...
        latents = self.prepare_latents(
            ...
            height,
            width,
            cloth_image_embeds.dtype,
            device,
            generator,
            latents,
        )

        # 6. prepare appearance flow feat.
        if warped_image is not None:
            cloth_aflow_inp = self.image_processor.preprocess(
                cloth_image, 
                height=256, 
                width=192)
            cloth_aflow_edge = self.compute_edge_map(cloth_aflow_inp.squeeze()).unsqueeze(0)
            pose_aflow_inp = self.image_processor.preprocess(
                pose_image,
                height=256,
                width=192)
            
            cloth_aflow_inp = cloth_aflow_inp.to(latents)
            cloth_aflow_edge = cloth_aflow_edge.to(latents)
            pose_aflow_inp = pose_aflow_inp.to(latents)
            
            warped_flow, warped_img, warped_mask = self.aflow(pose_aflow_inp, cloth_aflow_inp, cloth_aflow_edge)
            ...
            mask = fusion_tensor_mask(mask, warped_mask, iters=1)

        else:
            warped_image = self.image_processor.preprocess(warped_image)
            mask = transforms.ToTensor()(mask)
            mask = mask.unsqueeze(0)
            mask = mask.to(latents)
        ...
        pose_image = pose_image.to(latents)
        ...
        pose_model_latents = self.prepare_image_latents(pose_image)
        warped_img_latents = self.prepare_image_latents(warped_img)
        # flow_latents = torch.cat([warped_img_latents, pose_model_latents], dim=1)
        mask = mask.repeat(latents.shape[0], *([1] * (mask.dim() - 1)))
        # pose_fea = pose_fea.repeat(latents.shape[0], *([1] * (pose_fea.dim() - 1)))
        # aflow_fea = aflow_fea.repeat(latents.shape[0], *([1] * (aflow_fea.dim() - 1)))
        ...
        # warped_img = warped_img.repeat(latents.shape[0], *([1] * (warped_img.dim() - 1)))
        warped_img_latents = warped_img_latents.repeat(latents.shape[0], *([1] * (warped_img_latents.dim() - 1)))
        pose_model_latents = pose_model_latents.repeat(latents.shape[0], *([1] * (pose_model_latents.dim() - 1)))
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                # aflow_fea_input = torch.cat([aflow_fea] * 2) if self.do_classifier_free_guidance else aflow_fea
                warped_img_input = torch.cat([warped_img_latents] * 2) if self.do_classifier_free_guidance else warped_img_latents
                pose_model_input = torch.cat([pose_model_latents] * 2) if self.do_classifier_free_guidance else pose_model_latents
                # flow_latents_input = torch.cat([flow_latents] * 2) if self.do_classifier_free_guidance else flow_latents
                ...
                noise_pred = self.transformer_vton(hidden_states=torch.cat([latent_model_input, vton_model_input, mask_input], dim=1),
                            ...
                            ref_value=ref_value,
                            return_dict=False,
                            aflow_cond=warped_img_input,
                            pose_cond=pose_model_input)[0]
                # perform guidance
                if self.do_classifier_free_guidance:
